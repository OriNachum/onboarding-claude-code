...
> **Level: 🌿 Intermediate** | **Source:** [Hooks](https://docs.anthropic.com/en/docs/claude-code/hooks), [Skills](https://docs.anthropic.com/en/docs/claude-code/skills), [Sub Agents](https://docs.anthropic.com/en/docs/claude-code/sub-agents)
...
**Note:** Slash commands (like `/compact`, `/help`, and custom `/my-command`) are a subset of Skills. Built-in slash commands are covered in the [Built-ins](../beginner/built-ins.md) page. Custom slash commands are simply Skills with a name you invoke via `/`.

---

## The Three Automation Mechanisms

| Aspect | [Hooks](hooks.md) | [Skills](skills.md) | [Sub Agents](../expert/sub-agents.md) |
|---|---|---|---|
...
**"I want Claude to hand off work to a focused specialist"** → Use [Sub Agents](../expert/sub-agents.md). For example, delegate code reviews to a read-only reviewer agent, or run tests in an isolated debugging agent.
...
**"I want to know what Claude Code ships with out of the box"** → See [Built-ins](../beginner/built-ins.md) for all built-in slash commands, bundled skills, built-in hooks events, and built-in sub agents.
...
[Agent teams](../expert/team-mode.md) take this combination to the next level. Instead of one Claude session orchestrating skills, hooks, and sub agents, you get multiple full Claude instances — each with access to all your skills and hooks — working together with a shared task list and direct communication. One teammate can run your `/review` skill while another implements a fix, with hooks firing on both, and the teammates can discuss findings with each other without going through you.

Agent teams are still experimental, but they point to where this is heading: your skills define the playbooks, your hooks enforce the rules, and agent teams provide the workforce — all coordinating autonomously. See [Agent Teams](../expert/team-mode.md) for details.
...
[Ongoing Work](../expert/ongoing-work.md) for automated maintenance strategies.
...
- [HTTP Hooks](../expert/hooks-http.md) — Dedicated guide for HTTP endpoint hook handlers
- [Loop](loop.md) — Recurring command scheduling with /loop
- [Skills](skills.md) — Reusable prompts, coding standards, and slash commands
- [Sub Agents](../expert/sub-agents.md) — Specialist assistants with isolated context
- [Built-ins](../beginner/built-ins.md) — Everything Claude Code ships with out of the box
- [Agent Teams](../expert/team-mode.md) — Coordinated multi-agent sessions with shared task lists and direct messaging (experimental)
