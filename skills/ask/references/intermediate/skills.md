...
> **Level: 🌿 Intermediate** | **Source:** [Skills](https://docs.anthropic.com/en/docs/claude-code/skills)
...
**Frontmatter** — Optional YAML that controls skill behavior: `name`, `description`, `argument-hint`, `disable-model-invocation`, `user-invocable`, `allowed-tools`, `model`, `context`, `agent`, and `hooks`.
...
**Arguments** — Use `$ARGUMENTS` in your skill content to accept input. Running `/fix-issue 123` replaces `$ARGUMENTS` with `123`. Additional substitutions: `$ARGUMENTS[N]` or `$N` for indexed access (e.g., `$1` for the first argument), `${CLAUDE_SESSION_ID}` for the current session ID, and `${CLAUDE_SKILL_DIR}` for the skill's directory path.
