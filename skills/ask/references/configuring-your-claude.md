...
**"This task generates too much noise"** → Time to create a [Sub Agent](sub-agents.md). If test output, log analysis, or code review diffs are flooding your main conversation, delegate to a specialist that returns only a summary.
...
You've done the initial setup from [Setting Your Environment](setting-your-environment.md). You're working with Claude daily and noticing friction:
...
1. **CLAUDE.md** gives project context that every session loads ([Setting Your Environment](setting-your-environment.md))
