...
> **What's Happening:** [GitHub Actions with Claude Code](../github-actions.md) lets you run Claude as an automated process. The `allowed_tools` parameter scopes what Claude can do — read files, search, and write updates, but nothing destructive. Using Sonnet keeps costs low for a weekly maintenance job.
...
> **What's Happening:** This follows the [best practice](../best-practices.md) of giving Claude clear success criteria. "Check X against Y" is much more reliable than "make sure everything is up to date."
...
> **What's Happening:** [Ongoing work automation](../ongoing-work.md) extends naturally once you have the pattern. The same freshness-check approach works for any documentation that should stay in sync with code.
