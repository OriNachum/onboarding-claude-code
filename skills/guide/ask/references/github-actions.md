...
2. **Claude tool permissions** — what Claude is allowed to do (which tools it can call). Set via the `settings` input or by using `--dangerously-skip-permissions` in `claude_args` (see [Starting to Work](starting-to-work.md) for context on this flag).
...
See [Choosing Your Model](choosing-your-model.md) for model trade-offs.
...
See [Ongoing Work](ongoing-work.md) for more automated maintenance strategies.
...
| Input | Description |
|---|---|
| `claude_code_oauth_token` | API key or OAuth token for authentication (required) |
...
- See [Ongoing Work](ongoing-work.md) for automated maintenance strategies that use GitHub Actions
